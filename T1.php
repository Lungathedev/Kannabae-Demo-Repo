...
$placed = 'Placed'; 
   ...
   $date = $_GET['eDate'];
   $query="DELETE FROM orders WHERE idOrderConfirmation='".$placed."' ";
   $result=mysqli_query($conn,$query);
   $query="SELECT * FROM orders WHERE idOrderToken='".$authtoken."' ";
   $result=mysqli_query($conn,$query);
   ...
   $query="SELECT * FROM orders WHERE idOrders='".$order."' ";
   $result=mysqli_query($conn,$query);
   while($row=mysqli_fetch_array($result)){
   ...
   $query="SELECT * FROM deliverylog WHERE idOrderID='".$token."' AND idCustomerToken='".$code."' ";
   $result=mysqli_query($conn,$query);
   while($row=mysqli_fetch_array($result)){
   $ordernm[]=$row['idDelivery'];
   }
   $ordernmb=Min($ordernm); 
  
   $sql="SELECT * FROM orders WHERE idOrderConfirmation=? AND idOrderToken=? AND idOrders=? ";
   ...
   $sql='SELECT idTime FROM addresslog WHERE idOrderCustiCode="'.$code.'"';
   $stmt=mysqli_stmt_init($conn);
 if(!mysqli_stmt_prepare($stmt,$sql)){
...
$sql='SELECT idDate FROM addresslog WHERE idOrderCustiCode="'.$code.'"';
   $stmt=mysqli_stmt_init($conn);
 if(!mysqli_stmt_prepare($stmt,$sql)){
       header("Location:Signin.php?error=SQL2");
       exit();
...
$timeChange=date('H:i',strtotime('+180 minutes',strtotime($timeplaced)));
}
?>
<html>
<head>
...
<script>
  ...
  gtag('config', 'G-10BFE42JYZ');
    ...
    <meta name="description" content="Cannabis gifting website.">
    <meta name="keywords" content="kannabae">
    <meta name="author" content="K2021661320 (Pty) Ltd">
<title>Kannabae || Delivery status</title>
<link rel="stylesheet" 
...
<link rel="apple-touch-icon-precomposed" sizes="152x152" href="">
	...
	<span><a href="O1.php?auth=<?php echo $_GET['auth'];?>"><span style="color:gold;">[History]</span></a></span>
	</div>
...
<P id="payhide1"><strong>Subscription:</strong> <span style="color:green"> Active </span></p>
<P id="payhide2" style="display:none" ><strong>Subscription:</strong> <span style="color:orange"> Not active</span></p>
...
<P id="etahide1"><strong> Estimated time of arrival:</strong> Any time before - [<?php echo $timeChange; ?>] </p>
