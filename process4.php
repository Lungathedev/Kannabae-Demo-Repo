...
$query2='SELECT * FROM deliverylog WHERE idOrderID="'.$authtoken.'" ';
$result2=mysqli_query($conn,$query2);
while($row2=mysqli_fetch_array($result2)){
 $token2[]=$row2['idDelivery'];
}
$ordertoken2=Max($token2);

$sql="UPDATE orders SET idOrderConfirmation='Pending' WHERE idOrderToken='".$authtoken."' AND idOrders='".$ordertoken."' ";
mysqli_query($conn,$sql);
$sql1="UPDATE deliverylog SET idUsername='Pending' WHERE idOrderID='".$authtoken."' AND idDelivery='".$ordertoken."' ";
mysqli_query($conn,$sql1);
?>

<html>
<form action="SC1.php" method="get" id="form">
...
<script>
function auth(){
let auth = localStorage.getItem('canibu');
